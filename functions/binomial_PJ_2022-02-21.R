...
table(Total=dat$dog_total, Seen=dat$number_dogs_seen,useNA = "ifany")
...
# too few zeros compared to what we would expect... 
# in fact there are only two zeroes:
table(owner_recall_dat$owner_recall_vacc_no, useNA = "ifany")
# should the NA records
...
testDispersion(sim_resid) # underdispersed
testZeroInflation(sim_resid) 
table(cert_dat$dogs_vacc_cert, useNA = "ifany")
...
#' Set number of bootstrap samples (1000 for publication, which will take about 20 min)
#' If nsim is <= 0, not bootstrapping will be done. Instead, previously generated bootstrap
#' samples, stored as CSVs, will be loaded. 
nsim <- 0

#' Set random seed so that output is repeatable
set.seed(98734579)
...
if(nsim > 0) {
  system.time({
    boot_microchip <- bootMer(model_microchip, 
                              ...
                              nsim = nsim,re.form=NA, 
                              parallel = "multicore", ncpus = detectCores())$t
  })
  write.csv(boot_microchip, "boot_microchip.csv", quote = FALSE, row.names = FALSE)
} else boot_microchip <- read.csv("boot_microchip.csv")
...
pred_data$microchip_upper = apply(boot_microchip, 2, function(x) as.numeric(quantile(x, probs=.025)))
pred_data$microchip_lower = apply(boot_microchip, 2, function(x) as.numeric(quantile(x, probs=.975)))
pred_data$microchip_mean-apply(boot_microchip, 2, mean)
...
if(nsim > 0) {
  system.time({
    boot_recall <- bootMer(model_recall, 
                           ...
                           nsim = nsim,re.form=~(1|ward),
                           parallel = "multicore", ncpus = detectCores())$t
  })
  write.csv(boot_recall, "boot_recall.csv", quote = FALSE, row.names = FALSE)
} else boot_recall <- read.csv("boot_recall.csv")
pred_data$recall_mean <- predict(model_recall,newdata=pred_data,re.form=~(1|ward),type="response")
pred_data$recall_upper = apply(boot_recall, 2, function(x) as.numeric(quantile(x, probs=.025)))
pred_data$recall_lower = apply(boot_recall, 2, function(x) as.numeric(quantile(x, probs=.975)))
pred_data$recall_mean-apply(boot_recall, 2, mean)
...
if(nsim > 0) {
  ...
  write.csv(boot_cert, "boot_cert.csv", quote = FALSE, row.names = FALSE)
} else boot_cert <- read.csv("boot_cert.csv")
pred_data$cert_mean <- predict(model_cert,newdata=pred_data,re.form=NA,type="response")
pred_data$cert_upper = apply(boot_cert, 2, function(x) as.numeric(quantile(x, probs=.025)))
pred_data$cert_lower = apply(boot_cert, 2, function(x) as.numeric(quantile(x, probs=.975)))
pred_data$cert_mean-apply(boot_cert, 2, mean)
...
cols <- alpha(c("#BC3C29FF","#0072B5FF","#20854EFF","#7876B1FF"), alpha = 0.8)
show_col(cols)
...
legend(0.55,0.3,c("CPV","VLC","SVLC","DC"),title="Strategy:",col=cols,pch=15,bty="n",title.adj = 0)
legend(0.75,0.3,c("certificate","owner recall"),title="Method:",col = alpha(1, 0.5),pch=c(15,17),bty="n",title.adj = 0)
